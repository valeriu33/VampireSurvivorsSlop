...
let private drawCharacter (g: IGraphics) (dir: int) (body: int) (head: int) (scale: float) =
    ...
    fillColor g head |> ignore

    if facingCamera then
...
let private buildDirectional (r: IRenderer) (body: int) (head: int) (scale: float) =
    let tex = Array.zeroCreate 8
    for dir in 0 .. 7 do
        let g = newGraphics ()
        drawCharacter g dir body head scale
        tex.[dir] <- bake r g 2.0
        g.destroy ()
    { Textures = tex
